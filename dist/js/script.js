...
function init_page() {
  // arkk case ccustom js
  if (document.getElementById('fra')) {
    var run_a = function run_a() {
      $('#logo').delay(500).fadeIn(300).animate({
        bottom: '0px'
      }, {
        queue: false,
        duration: 1500
      });
      $('#body').delay(1500).fadeIn(300).animate({
        bottom: '0px'
      }, {
        queue: false,
        duration: 2500
      });
      $('#c, .glair').delay(3000).fadeIn(300).animate({
        bottom: '0px'
      }, {
        queue: false,
        duration: 2000
  ...
  if (document.getElementById('iframes')) {
    ...
    var of = row.children; // swap elements
  ...
  $('.case_link').hover(function () {
    if (!window.istouch) {
      var imageUrl = window.sitepath + "assets/images/" + $(this).data('img'); // $('#bg').fadeIn(600);
  ...
  $('.hov_exit').hover(function () {}, function () {
    if (!window.istouch) {
      $('#bg').fadeOut(600);
      setTimeout(function () {
        $('#bg_image').attr("src", '');
...
var transEffect = Barba.BaseTransition.extend({
  start: function start() {
    ...
    var vm = this;
    setTimeout(function () {
      vm.newContainerLoading.then(function (val) {
        return vm.fadein($(vm.newContainer));
      });
    }, 500);
    $('.ldBar').addClass('load');
  },
...
Barba.Pjax.start(); // page transistion end ___________________
...
var framePreload = function framePreload(url, delay) {
  ...
  if (document.images) {
    ...
    for (var i = 0; i < img.length; i++) {
      img[i].onload = function () {
        window.preload += fraction; // console.log(window.preload);
      };
    }
